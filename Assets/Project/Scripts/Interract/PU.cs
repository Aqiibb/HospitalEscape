...
public class PU : MonoBehaviour
{
    ...
    void Start()
    {
        LayerNumber = LayerMask.NameToLayer("IN"); //if your holdLayer is named differently make sure to change this ""
        ...
        if (Input.GetKeyDown(KeyCode.E)) //change E to whichever key you want to press to pick up
        {
            if (heldObj == null) //if currently not holding anything
            {
                ...
                if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, pickUpRange))
                {
                    ...
                    if (hit.transform.gameObject.tag == "IN")
                    {
    ...
    void RotateObject()
    {
        if (Input.GetKey(KeyCode.R)) // Hold R key to rotate, change this to whatever key you want
        {
            ...
            //float rotationSensitivity = 2f; // Adjust the rotation sensitivity as desired
            float rotationX = Input.GetAxis("Mouse X") * rotationSensitivity;
            float rotationY = Input.GetAxis("Mouse Y") * rotationSensitivity;

            // Duplicate the mouse X and Y values
            float XaxisRotation = rotationX;
            float YaxisRotation = rotationY;

            // Rotate the object depending on the duplicated mouse X-Y Axis
